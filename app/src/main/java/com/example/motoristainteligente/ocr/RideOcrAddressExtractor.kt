...
object RideOcrAddressExtractor {
    ...
    private const val STREET_PREFIX_REGEX = "(?:R(?:ua)?\\.?|Av(?:enida)?\\.?|Blvd\\.?|Boulevard|Estr\\.?|Estrada|Viela|Tv\\.?|Travessa|Al\\.?|Alameda|Pç\\.?|Praça|Rod\\.?|Rodovia|Lot\\.?|Loteamento|Conj\\.?|Conjunto|Res\\.?|Residencial|Cond\\.?|Condomínio)"
    ...
    private fun extractBestAddressFromSegment(segment: String): String {
        ...
        // Primeiro: tentar linhas com prefixo de rua (mais confiável)
        val prefixedLines = originalLines
            .map { sanitizeAddressCandidate(it) }
            .filter { it.length >= 4 }
            .filter { isValidAddressPrefix(it) }
            .filterNot { isLikelyNoiseAddressLine(it) }

        if (prefixedLines.isNotEmpty()) {
            val bestLine = prefixedLines.maxByOrNull { line ->
        ...
        // Segundo: para segmentos posicionais (entre pares min/km), aceitar linhas
        // que pareçam endereço mesmo sem prefixo de rua (ex: "Centro", "Bairro X", "Shopping Y")
        // A 99 frequentemente mostra apenas o nome do local/bairro sem "Rua"/"Av."
        val flexibleCandidates = originalLines
            .map { sanitizeAddressCandidate(it) }
            .filter { it.length >= 4 }
            .filterNot { isLikelyNoiseAddressLine(it) }
            .filter { isPlausibleAddressLine(it) }

        if (flexibleCandidates.isNotEmpty()) {
            val bestFlexible = flexibleCandidates.maxByOrNull { line ->
                val prefixScore = if (isValidAddressPrefix(line)) 4 else 0
                val digitScore = if (line.any { it.isDigit() }) 2 else 0
                val commaScore = if (line.contains(',')) 1 else 0
                val capitalScore = if (line.first().isUpperCase()) 1 else 0
                val sizeScore = line.length.coerceAtMost(60) / 15
                prefixScore + digitScore + commaScore + capitalScore + sizeScore
            }.orEmpty()

            if (bestFlexible.isNotBlank()) {
                return bestFlexible
            }
        }
        ...
        // Aceitar endereço genérico se tiver pelo menos uma palavra com letra maiúscula
        if (!isValidAddressPrefix(generic) && !isPlausibleAddressLine(generic)) return ""

        return sanitizeAddressCandidate(generic)
    }

    /**
     * Verifica se uma linha parece ser um endereço/local plausível (sem exigir prefixo Rua/Av).
     * Usado para segmentos posicionais da 99, onde o nome do local aparece
     * diretamente abaixo do par min/km.
     */
    private fun isPlausibleAddressLine(line: String): Boolean {
        if (line.length < 4) return false
        // Deve ter pelo menos uma letra
        if (!line.any { it.isLetter() }) return false
        // Não deve ser apenas números
        if (Regex("""^\d+[\d,\.\s]*$""").matches(line)) return false
        // Deve começar com letra maiúscula (nome de lugar) ou prefixo de rua
        val startsWithCapital = line.first().isUpperCase()
        val hasAddressPrefix = isValidAddressPrefix(line)
        if (!startsWithCapital && !hasAddressPrefix) return false
        // Não deve ser um token numérico do app (ex: "4,7 ★", "R$ 12,00")
        if (Regex("""^[\d,\.]+\s*[★☆%]""").containsMatchIn(line)) return false
        if (Regex("""^R\$""", RegexOption.IGNORE_CASE).containsMatchIn(line)) return false
        // Não deve ser texto curto genérico de UI
        val lowerLine = line.lowercase()
        val uiTokens = listOf(
            "aceitar", "recusar", "ignorar", "negocia", "confirmar",
            "corrida longa", "pop expresso", "corridas",
            "perfil premium", "perfil essencial", "taxa de deslocamento",
            "cartão verif", "prioridade", "favorito",
            "min", "km", "viagem de"
        )
        if (uiTokens.any { lowerLine == it || (lowerLine.length < 15 && lowerLine.startsWith(it)) }) return false
        return true
    }
