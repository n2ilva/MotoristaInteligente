...
class FirestoreManager(private val context: Context) {
            ...
            onReady = { userId ->
                resolveOfferLocation(
                    ...
                    db.collection(COLLECTION_REGIONAL_OFFERS)
                        ...
                        .addOnFailureListener {
                            Log.e(TAG, "Erro ao salvar oferta regional", it)
                            onComplete?.invoke(false)
                        }
    ...
    /**
     * Versão do saveRideOffer com callback de conclusão.
     * Usado no graceful shutdown para aguardar todas as escritas finalizarem.
     */
    fun saveRideOfferWithCallback(
        rideData: RideData,
        city: String? = null,
        neighborhood: String? = null,
        gpsLatitude: Double? = null,
    ...
    /**
     * Exclui uma oferta de corrida do Firestore pelo documentId.
     * Usado quando o motorista identifica uma leitura incorreta e quer remover a oferta.
     */
    fun deleteRideOffer(
        documentId: String,
        onSuccess: () -> Unit = {},
        ...
        if (documentId.isBlank()) {
            Log.w(TAG, "deleteRideOffer: documentId vazio, ignorando")
            onError(IllegalArgumentException("documentId vazio"))
            return
        }
        ...
        db.collection(COLLECTION_REGIONAL_OFFERS)
            ...
            .delete()
            .addOnSuccessListener {
                Log.d(TAG, "Oferta excluída com sucesso: $documentId")
                onSuccess()
            }
            .addOnFailureListener { e ->
                Log.e(TAG, "Erro ao excluir oferta: $documentId", e)
                onError(e)
            }
    }
